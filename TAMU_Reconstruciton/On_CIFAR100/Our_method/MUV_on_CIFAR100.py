...
def get_avg_recon_MSE(model, test_loader, reconstruction_function, data_name):
    ...
    M = Mine1(noise_size=256, sample_size=3*32*32)
    M.to(args.device)
...
def train_reconstructor(train_loader, reconstruction_function, args):
    ...
    reconstructor = resnet18(1, 3*32*32).to(args.device)
...
def calculate_MI(X, Z, Z_size, M, M_opt, args, ma_rate=0.001):
    ...
    z_bar = torch.randn((X.size(0), Z_size)).to(args.device)
