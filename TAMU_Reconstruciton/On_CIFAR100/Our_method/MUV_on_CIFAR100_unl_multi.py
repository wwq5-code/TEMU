...
def prepare_verification_dataset( erased_set_with_tri, erased_on_rem_with_tri, dataset2, er_on_remaining_set, remaining_set,args):

    erased_set_with_tri_list = list(erased_set_with_tri)
    erased_on_rem_with_tri_list = list(erased_on_rem_with_tri)
    dataset2_list = list(dataset2)

    combined_not_in_set = erased_set_with_tri_list + erased_on_rem_with_tri_list + dataset2_list

    er_on_remaining_set_list = list(er_on_remaining_set)
    remaining_set_list = list(remaining_set)



    for i in range(len(combined_not_in_set)):
        x, y = combined_not_in_set[i]

        combined_not_in_set[i] = x, 0

    for i in range(len(er_on_remaining_set_list)):
        x, y = er_on_remaining_set_list[i]

        er_on_remaining_set_list[i] = x, 1

    for i in range(len(er_on_remaining_set_list)*2):
        x, y = remaining_set_list[i]

        er_on_remaining_set_list.append((x, 1))


    indices_not_in = list(range(len(combined_not_in_set)))
    indices_in = list(range(len(er_on_remaining_set_list)))

    subset_no = Subset(combined_not_in_set, indices_not_in)
    subset_with = Subset(er_on_remaining_set_list, indices_in)
...
def linear_train(data_loader, vib_full_trained, model, loss_fn, args):
    ...
    for step, (x, y) in enumerate(data_loader):
        ...
        logits_z, logits_y, x_hat, mu, logvar = vib_full_trained(x, mode='with_reconstruction')
        x = x.view(x.size(0), -1)

        logits_y = model(x_hat.detach())  # (B, C* h* w), (B, N, 10)
        H_p_q = loss_fn(logits_y, y)
...
def get_grad_dataset(model, dataset_loader, unlearning_s_size):
    dim_z = 256
    temp_img = torch.empty(0, 3, 32, 32).float().to(args.device)
    temp_grad = torch.empty(0, dim_z*2).float().to(args.device)
    ...
    for step, (x, y) in enumerate(dataset_loader):
        ...
        for name, param in model.named_parameters():
            ...
            if name == 'encoder.linear_head.bias':
                # print(param)
                temp_img = torch.cat([temp_img, image], dim=0)
                t_grad = param.grad.view(1, dim_z)
                n, m = t_grad.size()
                B, C, H, W  = image.size()
                for i in range(B):
                    ...
                    noise = torch.randn(n, m).float().to(args.device)
                    ...
                    temp_grad = torch.cat([temp_grad, concatenated], dim=0)
...
def get_avg_recon_MSE(model, vib, test_loader, reconstruction_function, data_name):
    ...
    for grad, img in test_loader:
        ...
        grad = grad.view(grad.size(0), 2, 16, 16)
        img = img.view(img.size(0), -1)  # Flatten the images
        outputs = model(grad)
        x_hat = vib.reconstruction(outputs, img)
...
def train_reconstructor(vib, train_loader, reconstruction_function, args):
    ...
    reconstructor = resnet18(2, args.dimZ).to(args.device)
...
def infer_in_or_not(vib_full_trained, reconstructor_bac, classifier_model, er_with_trigger_train_loader):
    ...
    for batch_idx, (grad, img) in enumerate(er_with_trigger_train_loader):
        ...
        x_hat = vib_full_trained.reconstruction(outputs, img)
        x_hat = x_hat.view(x_hat.size(0), -1)
        # second, input the x_hat to classifier
        logits_y = classifier_model(x_hat.detach())
...
temp_grad, temp_img, total_loss_on_re = get_grad_dataset(copy.deepcopy(vib), dataloader_er_clean_on_re, unlearning_s_size)
...
temp_grad, temp_img, total_loss_tri_on_re = get_grad_dataset(copy.deepcopy(vib), dataloader_on_re_with_trigger, unlearning_s_size)
...
temp_grad, temp_img, total_loss_er_cl = get_grad_dataset(copy.deepcopy(vib), dataloader_er_clean, unlearning_s_size)
...
temp_grad, temp_img, total_loss_tri_er_cl = get_grad_dataset(copy.deepcopy(vib), dataloader_er_with_trigger, unlearning_s_size)
...
# Concatenate datasets and create a new loader
er_trainset = ConcatDataset([erased_on_rem_with_tri, erasing_set, erased_set_with_tri])
...
avg_mse_on_re = get_avg_recon_MSE(copy.deepcopy(reconstructor_er_re),copy.deepcopy(vib_full_trained), er_clean_on_re_train_loader, reconstruction_function, "er_clean_on_re")
print('avg_mse_on_re:', avg_mse_on_re/avg_mse_on_re, avg_mse_on_re)
avg_mse_tri_on_er = get_avg_recon_MSE(copy.deepcopy(reconstructor_er_on_rem),copy.deepcopy(vib_full_trained), er_on_rem_with_tri_train_loader, reconstruction_function, "er_with_tri_on_re")
print('avg_mse_tri_on_er:', avg_mse_tri_on_er/avg_mse_on_re, avg_mse_tri_on_er)
avg_mse_er_cl = get_avg_recon_MSE(copy.deepcopy(reconstructor_clean),copy.deepcopy(vib_full_trained), er_clean_train_loader, reconstruction_function, "er_clean")
print('avg_mse_er_cl:', avg_mse_er_cl/avg_mse_on_re,avg_mse_er_cl)
avg_mse_tri_er_cl = get_avg_recon_MSE(copy.deepcopy(reconstructor_bac),copy.deepcopy(vib_full_trained), er_with_trigger_train_loader, reconstruction_function, "er_with_trigger")
print('avg_mse_tri_er_cl:', avg_mse_tri_er_cl/avg_mse_on_re, avg_mse_tri_er_cl)
...
constructed_verify = prepare_verification_dataset(erased_set_with_tri, erased_on_rem_with_tri, dataset2,er_on_remaining_set, remaining_set,args)
