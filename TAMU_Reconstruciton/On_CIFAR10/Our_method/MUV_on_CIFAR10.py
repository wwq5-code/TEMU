...
def get_grad_dataset(model, dataset_loader):
    dim_z = 256
    temp_img = torch.empty(0, 3, 32, 32).float().to(args.device)
    ...
    for step, (x, y) in enumerate(dataset_loader):
        ...
        for name, param in model.named_parameters():
            ...
            if name == 'encoder.linear_head.bias':
                # print(param)
                temp_img = torch.cat([temp_img, image], dim=0)
                temp_grad = torch.cat([temp_grad, param.grad.view(1, dim_z)], dim=0)

    #optimizer.step()
    print("total_loss",total_loss)
    return temp_grad, temp_img
...
temp_grad, temp_img = get_grad_dataset(copy.deepcopy(vib), dataloader_er_clean_on_re)
...
print("er_clean_re finished")

temp_grad, temp_img = get_grad_dataset(copy.deepcopy(vib), dataloader_on_re_with_trigger)
...
print("er_clean_on_rem_with_tri finished")

temp_grad, temp_img = get_grad_dataset(copy.deepcopy(vib), dataloader_er_clean)
...
print("er_clean_set finished")

temp_grad, temp_img = get_grad_dataset(copy.deepcopy(vib), dataloader_er_with_trigger)
...
avg_mse = get_avg_recon_MSE(copy.deepcopy(reconstructor_er_re),copy.deepcopy(vib_full_trained), er_clean_on_re_train_loader, reconstruction_function, "er_clean_on_re")

avg_mse = get_avg_recon_MSE(copy.deepcopy(reconstructor_er_on_rem),copy.deepcopy(vib_full_trained), er_on_rem_with_tri_train_loader, reconstruction_function, "er_with_tri_on_re")

avg_mse = get_avg_recon_MSE(copy.deepcopy(reconstructor_clean),copy.deepcopy(vib_full_trained), er_clean_train_loader, reconstruction_function, "er_clean")

avg_mse = get_avg_recon_MSE(copy.deepcopy(reconstructor_bac),copy.deepcopy(vib_full_trained), er_with_trigger_train_loader, reconstruction_function, "er_with_trigger")
