...
import torch.optim
import torchvision
from torchvision.datasets import MNIST, CIFAR10, FashionMNIST, CIFAR100, CelebA, STL10
...
def add_trigger_new(add_backdoor, dataset, poison_samples, mode, args):
    ...
    for i in range(len(list_from_dataset_tuple)):
        if add_backdoor == 1:
            ...
            img = x * 255
            temp = 1 - x[:, -17:-2, -17:-2]
            x[:, -17:-2, -17:-2] = x[:, -17:-2, -17:-2] + temp * args.laplace_scale
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(128, 512 * 3 * 3)

        # Upscale to the desired dimensions using transposed convolutions
        self.deconv1 = nn.ConvTranspose2d(512, 256, kernel_size=4, stride=2, padding=1) # Output: 256 x 6 x 6
        self.deconv2 = nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1) # Output: 128 x 12 x 12
        self.deconv3 = nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1)  # Output: 64 x 24 x 24
        self.deconv4 = nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1)   # Output: 32 x 48 x 48
        self.deconv5 = nn.ConvTranspose2d(32, 3, kernel_size=4, stride=2, padding=1)    # Output: 3 x 96 x 96
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 512, 3, 3)  # Reshape to start the transposed convolutions
...
class ResNet(nn.Module):
    def __init__(self, in_channels, block_features, num_classes=10, headless=False):
        ...
        self.expand = nn.Sequential(
            nn.Conv2d(in_channels, block_features[0], kernel_size=3, stride=1, bias=False),
...
class VIB(nn.Module):
    def __init__(self, encoder, approximator, decoder):
        ...
        self.fc3 = nn.Linear(3 * 96 * 96, 3 * 96 * 96)  # output
    ...
    def reconstruction(self, logits_z, x):
        ...
        # print(x_m.shape, output_x.shape)
...
def init_vib(args):
    if args.dataset == 'MNIST':
        ...
        approximator = LinearModel(n_feature=args.dimZ)
        encoder = resnet18(3, args.dimZ * 2)  # resnet18(1, 49*2)
        decoder = LinearModel(n_feature=args.dimZ, n_output= 3* 96 * 96)# Decoder() #LinearModel(n_feature= 3 * 32 * 32, n_output=3 * 32 * 32) # args.dimZ resnet18(2, 3 * 32 * 32) #
        lr = args.lr
...
def get_avg_recon_MSE(model, vib, test_loader, reconstruction_function, data_name):
    ...
    for grad, img in test_loader:
      ...
      #  loss = reconstruction_function(x_hat*(x_inverse_m > 0).int(), img*(x_inverse_m > 0).int())
        ...
        B, Z_size = grad.shape
        x_hat = x_hat.view(x_hat.size(0), 3, 96, 96)
...
def train_reconstructor(vib, train_loader, reconstruction_function, args):
    ...
    for epoch in range(args.num_epochs_recon):
        for grad, img in train_loader:
            ...
            # output = output.view(output.size(0), 3, 96, 96)
...
args.beta = 0.0001
args.mse_rate = 0.000001 # 10
args.lr = 0.0005
args.dimZ = 20  # 40 #2
args.batch_size = 16
args.erased_local_r = 0.05 # the erased data ratio
args.unl_samples_size = 2
...
if args.dataset == 'MNIST':
    ...
    train_set = datasets.STL10(root='../data/STL10', split='train', download=True, transform=transform)
    test_set = datasets.STL10(root='../data/STL10', split='test', download=True, transform=transform)
    train_set_no_aug = datasets.STL10(root='../data/STL10', split='test', download=True, transform=transform)
...
erased_set_with_tri = add_trigger_new(add_backdoor, erasing_set, poison_samples, mode, args)
...
poison_trainset = ConcatDataset([re_re_set, erased_on_rem_with_tri, erasing_set, erased_set_with_tri])


# remaining dataset for retraining the model
# dataloader_remain = DataLoader(re_re_set, batch_size=args.batch_size, shuffle=True)
...
for epoch in range(args.num_epochs):
    ...
    vib, clean_acc_list, mse_list = vib_train(dataloader_remain, vib, loss_fn, reconstruction_function, args, epoch, clean_acc_list,
                                         mse_list, dataloader_remain, train_type, dataloader_er_clean, dataloader_er_with_trigger)
...
recovery_trainset = ConcatDataset([er_clean_on_re_set, er_on_rem_with_tri_set])
...
classifier_model = LinearModel(n_feature=3 * 96 * 96, n_output=2).to(args.device)
...
constructed_verify = ConcatDataset([er_clean_on_re_veri_set, er_on_rem_with_tri_veri_set])
