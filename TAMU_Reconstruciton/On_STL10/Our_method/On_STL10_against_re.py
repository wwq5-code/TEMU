...
def unlearning_frkl(vibi_f_frkl, optimizer_frkl, vibi, epoch_test_acc, dataloader_erase, dataloader_remain, loss_fn,
    ...
    temp_acc = []
    temp_back = []
    unl_rate = args.unlearn_learning_rate
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            KLD_element_1 = mu_e.pow(2).add_(logvar_e.exp()).mul_(-1).add_(1).add_(logvar_e).to(args.device)
            KLD_mean_1 = torch.mean(KLD_element_1).mul_(-0.5).to(args.device)
            ...
            unlearning_item = args.kld_r * KLD_mean.item() - args.unlearn_bce_r * BCE.item() + args.unlearn_ykl_r * kl_f_e.item() - args.unlearn_learning_rate * H_p_q.item() - args.reverse_rate * (
                        log_z.item() + log_y.item())
            ...
            exp_log_p_y = torch.exp(-H_p_q)

            #args.kld_r * KLD_mean_f +
            if train_type == 'VIBU':
                #loss = args.kld_r * KLD_mean + args.unl_r_for_bayesian * (- H_p_q) - args.reverse_rate * (log_z + log_y)

                #loss = args.kld_r * KLD_mean - args.unlearn_bce_r * BCE + args.unlearn_ykl_r * kl_f_e - unl_rate * H_p_q - args.reverse_rate * (log_z + log_y)
                loss = args.kld_r * KLD_mean +args.unlearn_bce_r * KLD_mean_1 + args.unlearn_ykl_r * H_p_q_f +  args.unlearn_learning_rate *(-H_p_q) - args.reverse_rate * (log_z + log_y)# - args.reverse_rate * (log_z + log_y) #exp_log_p_y  #
            ...
            metrics = {
                ...
                'loss': loss.item(),
                'BCE': BCE.item(),
                'H_p_q_f': H_p_q_f.item(),
                'H(p,q)': H_p_q.item(),
                'kl_f_e': kl_f_e.item(),
                'H_p_q2': H_p_q2.item(),
                'exp_log_p_y': exp_log_p_y.item(),
                # '1-JS(p,q)': JS_p_q,
...
args.reverse_rate = 0.001* args.unlearn_learning_rate
args.kld_r = 1/1596200.641 * args.unlearn_learning_rate
args.unlearn_ykl_r = args.unlearn_learning_rate* 0.1

args.unlearn_bce_r = 0*args.unlearn_learning_rate * args.kld_r
