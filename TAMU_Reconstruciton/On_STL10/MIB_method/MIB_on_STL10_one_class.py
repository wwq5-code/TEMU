...
def add_trigger(add_backdoor, data, targets, poison_samples, mode, feature_extra):
    ...
    for i in range(len(data)):
        if add_backdoor == 1:
            ...
            # image_steg = generate_gray_laplace_noise(new_data[i])
    ...
    return new_data,  torch.Tensor(new_targets).long() #, targets #
...
# reshaped_data = data_reshape_to_np(erasing_set,dataname="MNIST")
# poison_samples = len(erasing_set.data)
# mode = "train"
feature_extra = SimpleCNN().cuda()
# erased_data_tri, erased_target_tri = add_trigger(add_backdoor, erasing_set.data, erasing_set.targets, poison_samples, mode, feature_extra)

## add the trigger to one class

poison_samples = erasing_size
base_label=1
trigger_label = 2
add_backdoor=1 # =1 add backdoor , !=1 not add
erased_data_tri, erased_target_tri = create_backdoor_train_dataset(dataname=args.dataset, train_data=train_set,
...
x = torch.tensor(erased_data_tri[0])
# print(x)
x = x.cpu().data
x = x.clamp(0, 1)
if args.dataset == "MNIST":
...
print('poison_data.shape', erased_target_tri.shape)

data = torch.cat([erased_data_tri.to(args.device), data_reshape.to(args.device)], dim=0)
targets = torch.cat([erased_target_tri.to(args.device), remaining_set.targets.to(args.device)], dim=0)
...
erased_dataset_with_trigger = Data.TensorDataset(erased_data_tri, erased_target_tri)
dataloader_erased_with_trigger = DataLoader(erased_dataset_with_trigger, batch_size=args.batch_size, shuffle=True)
...
erased_data_no_tri = erasing_set.data
labels_no_tri = torch.zeros_like(erasing_set.targets)

labels_tri = torch.ones_like(erased_target_tri)

data = torch.cat([erased_data_no_tri.to(args.device), erased_data_tri.to(args.device)], dim=0)
targets = torch.cat([labels_no_tri.to(args.device), labels_tri.to(args.device)], dim=0)


dataset_w_wo_tri = Data.TensorDataset(data, targets)
dataloader_erased_w_wo_tri = DataLoader(dataset_w_wo_tri, batch_size=args.batch_size, shuffle=True)
...
acc = num_correct / num_total
acc = round(acc, 5)
print(f'accuracy on test dataset:  {acc:.4f}') # this is a test for data without add the watermark.
