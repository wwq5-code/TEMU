...
def get_grad_dataset(model, dataset_loader, erasing_size):
    ...
    for step, (x, y) in enumerate(dataset_loader):
        ...
        grad_dict = {}
        for (name, param), grad in zip(model.named_parameters(), first_order_grads):
            grad_dict[name] = grad


        # Pick one gradient to calculate second-order derivative
        first_order_grad = grad_dict['encoder.linear_head.bias'] # Example: gradient w.r.t weights of the first layer

        # Second-order gradient
        second_order_grads = torch.autograd.grad(first_order_grad.sum(), model.parameters())


        second_grad_dict = {}
        for (name, param), second_grad in zip(model.named_parameters(), second_order_grads):
            second_grad_dict[name] = second_grad

        # print('First-order gradient:', first_order_grad)
        # print('Second-order gradient:', second_grad_dict['encoder.linear_head.bias'])

        second_order_grad = second_grad_dict['encoder.linear_head.bias']
        ...
        for name, param in model.named_parameters():
            ...
            if name == 'encoder.linear_head.bias':
                ...
                temp_img = torch.cat([temp_img, image], dim=0)
                t_grad = first_order_grad.view(1, -1).detach() + second_order_grad.view(1,-1).detach() * first_order_grad.view(1, -1).detach()
