...
def get_avg_recon_MSE(model, test_loader, reconstruction_function, data_name):
    # Calculate average MSE over the test dataset

    M = Mine1(noise_size=256, sample_size=28*28)
    M.to(args.device)
    ...
    for grad, img in test_loader:
        ...
        outputs = model(grad)
        loss = reconstruction_function(outputs, img)
        total_mse += loss.item()
        grad = grad.view(grad.size(0), -1)
        B, Z_size = grad.shape
        mi = calculate_MI(img.detach(), grad.detach(), Z_size, M, M_opt, args, ma_rate=0.001)
        if mi < 0:
            mi = calculate_MI(img.detach(), grad.detach(), Z_size, M, M_opt, args, ma_rate=0.001)

    average_mse = total_mse / len(test_loader)
    print(f'{data_name} Average mutual information: {mi}')
    print(f'{data_name} Average Decoding MSE: {average_mse}')
    return average_mse

def train_reconstructor(train_loader, reconstruction_function, args):
    # init reconsturctor
    # reconstructor = LinearModel(n_feature=40, n_output=28 * 28).to(args.device)
    reconstructor = resnet18(1, 28*28).to(args.device)
    ...
    for epoch in range(epochs):
        for grad, img in train_loader:
            ...
            loss = reconstruction_function(output, img)
...
reconstructor_er_re = train_reconstructor(er_clean_on_re_train_loader, reconstruction_function, args)

reconstructor_er_on_rem = train_reconstructor(er_on_rem_with_tri_train_loader, reconstruction_function, args)

reconstructor_clean = train_reconstructor(er_clean_train_loader, reconstruction_function, args)

reconstructor_bac = train_reconstructor(er_with_trigger_train_loader, reconstruction_function, args)

avg_mse = get_avg_recon_MSE(copy.deepcopy(reconstructor_er_re), er_clean_on_re_train_loader, reconstruction_function, "er_clean_on_re")

avg_mse = get_avg_recon_MSE(copy.deepcopy(reconstructor_er_on_rem), er_on_rem_with_tri_train_loader, reconstruction_function, "er_with_tri_on_re")

avg_mse = get_avg_recon_MSE(copy.deepcopy(reconstructor_clean), er_clean_train_loader, reconstruction_function, "er_clean")

avg_mse = get_avg_recon_MSE(copy.deepcopy(reconstructor_bac), er_with_trigger_train_loader, reconstruction_function, "er_with_trigger")
